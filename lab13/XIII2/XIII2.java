...
public class XIII2 {
	private static final Port av = new SeaPort();
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		try (PrintStream fl = new PrintStream(new File(sp+"PDS2122.txt"))) {
	...
	private static void question2(PrintStream out) {
		...
		CargoShip cs = new CargoShip("S999", "Transportador", 50.0, 3);
		PassengerShip ps1 = (PassengerShip) av.remove("P54");
		PassengerShip ps2 = (PassengerShip) av.remove("P35");
		
		cs.addPassengerShip(ps1);
		cs.addPassengerShip(ps2);
	...
	private static void question3(PrintStream out) {
		...
		Port rp = RiverPort.createRiverPort(logger);
		
		rp.add("C01", new CargoShip("S001", "Pequeno", 5.0));
		rp.add("C02", new CargoShip("S002", "Grande", 15.0));
		rp.add("P01", new PassengerShip("S003", "Rio Douro", 30));
		rp.exists("C01");
		rp.remove("P01");
